...
from improved_dual_hand_model_v3 import ImprovedDualHandASDiffusionModel
...
class ImprovedDualHandTrainer:
    def __init__(self, encoder_params, decoder_params, diffusion_params, 
        ...
        # Use the improved model with architectural coordination
        self.model = ImprovedDualHandASDiffusionModel(
            encoder_params, decoder_params, diffusion_params, self.num_classes, self.device
        )
        print('Improved Dual Hand Model Size: ', sum(p.numel() for p in self.model.parameters()))
        
        print("🎯 Using Improved Architecture with:")
        print("   - Multi-scale cross-hand feature fusion")
        print("   - Architectural coordination at layers [3, 6, 9]")
        print("   - Learnable coordination gates")
    ...
    def train(self, train_train_dataset, train_test_dataset, test_test_dataset, loss_weights, class_weighting, soft_label,
        ...
        # Optimized training settings for coordination
        optimizer = optim.AdamW(self.model.parameters(), lr=learning_rate, weight_decay=weight_decay*0.5, eps=1e-8)
        
        # Learning rate scheduler with warmup for coordination training
        warmup_steps = min(50, num_epochs // 10)
        scheduler = optim.lr_scheduler.LambdaLR(
            optimizer, 
            lr_lambda=lambda epoch: min(1.0, (epoch + 1) / warmup_steps) if epoch < warmup_steps 
                                   else 0.5 * (1 + np.cos(np.pi * (epoch - warmup_steps) / (num_epochs - warmup_steps)))
        )
        ...
        best_coord_ratio = 0.0  # Track coordination ratio improvement
            ...
            if 'latest.pt' in os.listdir(result_dir):
                if os.path.getsize(os.path.join(result_dir, 'latest.pt')) > 0:
                    try:
                        ...
                        if 'best_coord_ratio' in saved_state:
                            best_coord_ratio = saved_state['best_coord_ratio']
        ...
        print(f"\n🏋️ Starting Improved Architecture Training for {num_epochs} epochs")
        ...
        for epoch in range(restore_epoch+1, num_epochs):
            ...
            for batch_idx, data in enumerate(epoch_progress):
                ...
                for k, v in loss_dict.items():
                    if k.startswith('coord_'):
                        coord_loss_total += v * 2.0  # Increased weight for coordination
                    else:
                        base_key = k.replace('lh_', '').replace('rh_', '')
                        if base_key in loss_weights:
                            total_loss += loss_weights[base_key] * v

                # Add coordination losses with higher weight
                total_loss += coord_loss_total
                epoch_coord_loss += coord_loss_total.item()

                # Update progress bar
                epoch_progress.set_postfix({
                    'Total Loss': f'{total_loss.item():.4f}',
                    'Coord Loss': f'{coord_loss_total.item():.4f}',
                ...
                if result_dir and step % 10 == 0:
                    ...
                    logger.add_scalar('Train-Coordination-Total', coord_loss_total.item(), step)
                    logger.add_scalar('Learning-Rate', scheduler.get_last_lr()[0], step)

                total_loss /= batch_size
                total_loss.backward()
                
                # Enhanced gradient clipping for coordination training
            ...
            epoch_coord_loss /= len(train_train_dataset)

            print(f'\nEpoch {epoch} Summary:')
            print(f'  Total Loss: {epoch_running_loss:.6f}')
            print(f'  Coordination Loss: {epoch_coord_loss:.6f}')
            ...
            if result_dir:
                state = {
                    ...
                    'best_coord_ratio': best_coord_ratio
                }
            ...
            if epoch % log_freq == 0:
                ...
                for mode in ['decoder-agg']: 
                    ...
                    # Calculate coordination score and coordination ratio
                    coordination_score = (test_result_dict.get('Avg_Acc', 0) + 
                                        test_result_dict.get('Avg_F1@50', 0)) / 2.0
                    
                    # Calculate estimated coordination ratio (proxy metric)
                    coord_ratio = min(test_result_dict.get('Avg_Acc', 0) / 45.0, 1.0)  # Normalize against target
                    ...
                    print(f'  Estimated Coord Ratio: {coord_ratio:.4f}')
                    ...
                    if coord_ratio > best_coord_ratio:
                        best_coord_ratio = coord_ratio
                        if result_dir:
                            torch.save(self.model.state_dict(), f'{result_dir}/best_coord_ratio_model.model')
                            print(f'🎯 New best coordination ratio model saved! Ratio: {best_coord_ratio:.4f}')
        ...
        print(f"   Best Coordination Ratio: {best_coord_ratio:.4f}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--improved_architecture', action='store_true', default=True,
                       help='Use improved architecture with cross-hand coordination')
    ...
    print("🚀 IMPROVED DUAL-HAND ARCHITECTURE TRAINING")
    print("=" * 60)
    print(args.config)
    print("Improved architecture with cross-hand coordination enabled")
    print("Key improvements:")
    print("  - Multi-scale cross-hand feature fusion")
    print("  - Architectural coordination at encoder layers")
    print("  - Strong coordination mismatch penalties")
    print("  - Learnable coordination gates")
    print("=" * 60)
    ...
    trainer = ImprovedDualHandTrainer(dict(encoder_params), dict(decoder_params), dict(diffusion_params), 
    ...
    trainer.train(train_train_dataset, train_test_dataset, test_test_dataset, 
        ...
        result_dir=os.path.join(result_dir, naming + "_improved_arch"), 
