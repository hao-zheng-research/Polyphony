...
class AugmentedDualHandDataset(DualHandVideoFeatureDataset):
    """
    Dataset with data augmentation applied
    """
...
class DualHandTrainerWithDataAug:
    def __init__(self, encoder_params, decoder_params, diffusion_params, 
        ...
        # Use original dual hand model (no architectural changes)
        self.model = DualHandASDiffusionModel(encoder_params, decoder_params, diffusion_params, self.num_classes, self.device)
        print('Dual Hand Model with Data Augmentation Size: ', sum(p.numel() for p in self.model.parameters()))
        ...
        print("🔧 Data Augmentation Features:")
        print("   ✅ Temporal jittering: Slightly shifts action boundaries")
        print("   ✅ Action substitution: Replaces similar actions occasionally")
        print("   ✅ Confidence smoothing: Applies label smoothing")
    ...
    def train(self, train_train_dataset, train_test_dataset, test_test_dataset, loss_weights, class_weighting, soft_label,
        ...
        for epoch in range(restore_epoch+1, num_epochs):
            ...
            for _, data in enumerate(train_train_loader):
                ...
                # Apply data augmentation during training (if not already applied in dataset)
                if self.model.training and not hasattr(train_train_dataset, 'enable_augmentation'):
                    (lh_label_aug, rh_label_aug, 
                     lh_boundary_aug, rh_boundary_aug, _) = self.data_augmentation.apply_augmentation(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--result_suffix', type=str, default='data_aug',
    ...
    print(args.config)
    print(all_params)
    print(f"🔧 Dual Hand Training with Data Augmentation:")
    ...
    print(f"  - Original Architecture: Proven stability")
    ...
    trainer = DualHandTrainerWithDataAug(dict(encoder_params), dict(decoder_params), dict(diffusion_params), 
