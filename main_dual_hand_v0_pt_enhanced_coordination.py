...
class EnhancedDualHandTrainer:
    def __init__(self, encoder_params, decoder_params, diffusion_params, 
        ...
        print("🚀 Using Enhanced Coordination Loss System")
        print("   - Coordination Enforcement Loss (addresses 24.3% coordination gap)")
        print("   - Enhanced Transition Synchronization Loss (addresses 0.04 sync ratio)")
        print("   - Null Coordination Loss (addresses null over-prediction)")
        print("   - Action Pair Consistency Loss (addresses specific confusions)")
        print("   - Confidence-based Coordination Loss (addresses uncertainty)")
    ...
    def train(self, train_train_dataset, train_test_dataset, test_test_dataset, loss_weights, class_weighting, soft_label,
        ...
        # Enhanced optimizer with lower learning rate for stability
        optimizer = optim.AdamW(self.model.parameters(), lr=learning_rate*0.8, weight_decay=weight_decay, eps=1e-8)
        ...
        step = 1
        best_avg_acc = 0.0
        best_coordination_score = 0.0  # Track coordination improvement
        ...
        print(f"\n🏋️ Starting Enhanced Coordination Training for {num_epochs} epochs")
        ...
        for epoch in range(restore_epoch+1, num_epochs):
            ...
            for batch_idx, data in enumerate(epoch_progress):
                ...
                # Add enhanced coordination losses to total
                coordination_total = enhanced_losses['total_enhanced_coordination']
                total_loss += coordination_total
                
                epoch_coordination_loss += coordination_total.item()

                # Update progress bar
                epoch_progress.set_postfix({
                    'Total Loss': f'{total_loss.item():.4f}',
                    'Coord Loss': f'{coordination_total.item():.4f}',
                    'LR': f'{scheduler.get_last_lr()[0]:.6f}'
                })

                if result_dir and step % 10 == 0:  # Log every 10 steps
                    # Log standard losses
                    for k, v in loss_dict.items():
                        base_key = k.replace('lh_', '').replace('rh_', '')
                        if base_key in loss_weights:
            ...
            print(f'  Coordination Loss: {epoch_coordination_loss:.6f}')
            ...
            if epoch % log_freq == 0:
                ...
                for mode in ['decoder-agg']: 
                    ...
                    # Calculate coordination score (custom metric)
                    ...
                    # Save best model based on coordination score
...
if __name__ == '__main__':
    ...
    parser.add_argument('--enhanced_coordination', action='store_true', default=True,
                       help='Use enhanced coordination loss system')
    ...
    print("🚀 ENHANCED DUAL-HAND COORDINATION TRAINING")
    print("=" * 60)
    print(args.config)
    print("Enhanced coordination system enabled")
    print("Addressing specific coordination issues:")
    print("  - 24.3% coordination gap (41.6% vs 65.9% GT)")
    print("  - 0.04 transition sync ratio (should be ~1.0)")
    print("  - Null over-prediction (27.2% vs 14.8% GT)")
    print("  - Action-specific confusions")
    print("=" * 60)
    ...
    trainer.train(train_train_dataset, train_test_dataset, test_test_dataset, 
        ...
        result_dir=os.path.join(result_dir, naming + "_enhanced"), 
